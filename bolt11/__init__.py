...
def decode(invoice) -> str:
    ...
    while data.pos != data.len:
        ...
        tag, tag_data, data = pull_tagged(data)
        data_length = len(tag_data) / 5

        if tag == 'd':
            tags['description'] = trim_to_bytes(tag_data).decode('utf-8')

        elif tag == 'h' and data_length == 52:
            tags['description_hash'] = trim_to_bytes(tag_data).hex()

        elif tag == 'p' and data_length == 52:
            tags['payment_hash'] = trim_to_bytes(tag_data).hex()

        elif tag == 'x':
            tags['expiry'] = tag_data.uint

        elif tag == 'n':
            tags['payee'] = trim_to_bytes(tag_data).hex()

        elif tag == 's':
            tags['secret'] = trim_to_bytes(tag_data).hex()
            ...
            while stream.pos + 408 < stream.len:
                ...
                tags['route_hints'].append({
                    'short_channel_id' : readable_scid(stream.read(64).intbe),
    ...
    tags['expiry'] = tags['expiry'] + tags['date']
...
def trim_to_bytes(data):

    return (
        data.tobytes()[:-1] if data.len % 8 != 0 else data.tobytes()
    )

def readable_scid(short_channel_id):
